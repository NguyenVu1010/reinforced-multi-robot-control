...
class RosHandler:
    def __init__(self, launch_command, robot_names):
        ...
        with DATA_LOCK:
            ...
            GLOBAL_PATHS_DATA = {name: [] for name in robot_names}
    ...
    def register_subscribers(self):
        ...
        for name in self.robot_names:
            ...
            self.subscribers.append(sub)
        for name in self.robot_names:
            path_topic = f'/paths/{name}/planned_path'  # hoặc topic bạn đang dùng
            sub = rospy.Subscriber(path_topic, Path, self.path_callback, callback_args=name, queue_size=1)
            self.subscribers.append(sub)
        ...
        with DATA_LOCK:
            ...
            for name in self.robot_names:
                try:
                    ...
                    continue
    def simplify_path(self,path_points, epsilon=0.1):
        """Simplifies a path using the Ramer-Douglas-Peucker algorithm."""
        if not path_points or len(path_points) < 3:
            return path_points
        
        # RDP yêu cầu numpy array (Numpy được import ở trên)
        simplified_path = rdp(np.array(path_points), epsilon=epsilon)
        return simplified_path.tolist() # Chuyển lại thành list để tương thích JSON
    def path_callback(self, msg, robot_name):
        """Callback để nhận path từ topic ROS, lưu path cho từng robot."""
        if not msg.poses:
            return
        ...
        for pose_stamped in msg.poses:
            ...
            path.append([pos.x, pos.y])  # Lưu gọn dạng list [x, y]

        with DATA_LOCK:
            GLOBAL_PATHS_DATA[robot_name] = self.simplify_path(path,0.1)
